...
* **Frontend React (Fases 2, 3 e 4):** React.js (via Vite), JSX, Componentização (`ProdutoCard`), prop `children` e UI responsiva.
* **Integrações:** Consumo assíncrono de dados via `fetch` simulando uma API a partir do arquivo `produtos.json` (gerenciado com `useEffect`).
* **Persistência de Dados:** Armazenamento contínuo do acervo utilizando a API do `localStorage` do navegador.
...
* [x] **Atividade 4: Estado, Efeitos e Assincronismo**
    Gerenciamento de estado (`useState`) para formulários, filtros e controle da lista. Consumo assíncrono de dados via `fetch` na inicialização (`useEffect`) e persistência total das adições e remoções de estoque no `localStorage`.
...
### Fases 2, 3 e 4: Rodando a Aplicação React (Vite)
